...
public class TraineeService {
    ...
    // Returns all trainings belonging to a trainee, fully loaded with trainer and user data
    // Called BEFORE deleteTrainee() so the cascade has not yet removed the records
    // Used by GymFacade to build DELETE notifications for the report service
    public List<Training> getTrainingsByUsername(String username) {
        Trainee trainee = traineeRepository.findByUserUsername(username)
                .orElseThrow(() -> new NoSuchElementException("Trainee not found: " + username));
        return trainingRepository.findByTraineeId(trainee.getId());
    }
