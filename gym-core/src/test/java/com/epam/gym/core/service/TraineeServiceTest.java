...
class TraineeServiceTest {
    ...
    void createTrainee_BuildUserAndTrainee() {
        ...
        Trainee saved = traineeWithUser("John.Doe", "$2a$encoded", true);
    ...
    @Test
    void getTrainingsByUsername_ReturnsTrainingsForExistingTrainee() {
        Trainee trainee = traineeWithUser("John.Doe", "pass", true);
        trainee.setId(java.util.UUID.randomUUID());
        Training training = new Training();
        when(traineeRepository.findByUserUsername("John.Doe")).thenReturn(Optional.of(trainee));
        when(trainingRepository.findByTraineeId(trainee.getId())).thenReturn(List.of(training));

        List<Training> result = traineeService.getTrainingsByUsername("John.Doe");
    ...
    @Test
    void getTrainingsByUsername_ThrowsWhenTraineeNotFound() {
        when(traineeRepository.findByUserUsername("unknown")).thenReturn(Optional.empty());

        assertThrows(NoSuchElementException.class,
                () -> traineeService.getTrainingsByUsername("unknown"));
        verify(trainingRepository, never()).findByTraineeId(any());
    }
